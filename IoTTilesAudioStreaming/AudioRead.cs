...
namespace IoTTilesAudioStreaming
{
    public class AudioRead
    {
        ...
        public void StartRecording(SslStream sslStream)
            //FixedSizedQueue<byte[]> queue)
        {
            if (_isRecording)
                return;

            _waveIn = new WasapiLoopbackCapture();
            _writer = new LameMP3FileWriter(sslStream, _waveIn.WaveFormat, 320);
    ...
    public class QueueStream : Stream
    {
        ...
        {
            _queue.Enqueue(buffer.Skip(offset).Take(count).ToArray());
        }
    }
}
